...
export async function generateExcelReport(results: TestResultRecord[], reportPath?: string) {
  const targetPath = reportPath || path.join(__dirname, '../../reports/api-unit-report-300.xlsx');
  ...
  const worksheet = workbook.addWorksheet('API Unit Test Results');
  ...
  // Header styling (Cyan / Teal accent for API Unit Tests)
  const headerRow = worksheet.getRow(1);
  headerRow.font = { bold: true, color: { argb: 'FFFFFF' }, size: 11 };
  headerRow.fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: '0F766E' }, // Teal / Cyan fgColor
  };
